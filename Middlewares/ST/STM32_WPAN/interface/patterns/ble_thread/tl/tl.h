...
typedef struct
{
  uint32_t *next;
  uint32_t *prev;
} __attribute__((packed)) TL_PacketHeader_t;
...
typedef struct
{
  ...
  uint8_t   numcmd;
  uint16_t  cmdcode;
} __attribute__((packed)) TL_CsEvt_t;
...
typedef struct
{
  ...
  uint16_t  cmdcode;
  uint8_t   payload[2];
} __attribute__((packed)) TL_CcEvt_t;
...
typedef struct
{
  uint16_t  subevtcode;
  uint8_t   payload[2];
} __attribute__((packed)) TL_AsynchEvt_t;
...
typedef struct
{
  ...
  uint8_t   plen;
  uint8_t   payload[2];
} __attribute__((packed)) TL_Evt_t;

typedef struct
{
  uint8_t   type;
  TL_Evt_t  evt;
} __attribute__((packed)) TL_EvtSerial_t;
...
} __attribute__((packed)) TL_EvtPacket_t;
...
typedef struct
{
  ...
  uint8_t   plen;
  uint8_t   payload[255];
} __attribute__((packed)) TL_Cmd_t;

typedef struct
{
  uint8_t   type;
  TL_Cmd_t  cmd;
} __attribute__((packed)) TL_CmdSerial_t;
...
typedef struct
{
  ...
  uint16_t  length;
  uint8_t   acl_data[1];
} __attribute__((packed)) TL_AclDataSerial_t;

typedef struct
{
  TL_PacketHeader_t  header;
  TL_AclDataSerial_t   AclDataSerial;
} __attribute__((packed)) TL_AclDataPacket_t;
