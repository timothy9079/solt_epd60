...
#define PRINT_MESG_DBG(...)     do{printf("\r\n [%s][%s][%d] ", DbgTraceGetFileName(__FILE__),__FUNCTION__,__LINE__);printf(__VA_ARGS__);printf("\r\n");}while(0);
#else
...
extern void DbgTrace_mem_print_bin(		  const uint8_t *title, const uint8_t* buffer, uint32_t length);
  /**
